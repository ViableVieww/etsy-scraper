...
import { getFileData, purgeDuplicates, writeTextToS3, writeToFile } from './utils.js';
  ...
  let counter = 0;
    ...
    while (!isLastPage) {
      try {
        ...
        if (everbeeData.results.length > 0 && page < 5) {
          ...
          const listingS3Location = await writeTextToS3(`etsy_listings/${month}-${day}-${year}.json`, getFileData('listings.json'));
          console.log('\nWrote Listings: ', listingS3Location);

          purgeDuplicates('shops.json');
          console.log('\nPurged Shop Duplicates');

          const shopsS3Location = await writeTextToS3(`etsy_shops/${month}-${day}-${year}.json`, getFileData('shops.json'));
